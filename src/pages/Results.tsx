...
import { QuizResult, QuizData } from '@/lib/genlayer';

const Results = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { result, quizData, topic, difficulty, answers } = location.state as { 
    result: QuizResult;
    quizData: QuizData;
    topic: string;
    difficulty: string;
  ...
  const scoreNum = parseInt(result.total_score.split('/')[0]);
  const percentage = parseInt(result.percentage);
    ...
    <div className="min-h-screen bg-background bg-halftone flex flex-col">
      ...
      <div className="relative pt-16 pb-8 px-6 text-center">
        ...
        <div className="relative z-10">
          ...
          <p className="text-muted-foreground mb-6">{topic} • {difficulty}</p>
          
          <div className={`text-7xl font-black ${getScoreColor()} mb-2`}>
            {result.percentage}
          </div>
          <p className="text-2xl font-bold text-foreground">
            {result.total_score} correct
          </p>
        </div>
      </div>
      ...
      <div className="px-6 mb-8">
        <div className="max-w-2xl mx-auto grid grid-cols-3 gap-4">
          <div className="bg-card rounded-2xl p-4 text-center border-2 border-foreground/10">
            ...
            <p className="text-2xl font-black text-foreground">{scoreNum}</p>
            <p className="text-sm text-muted-foreground">Correct</p>
          </div>
          <div className="bg-card rounded-2xl p-4 text-center border-2 border-foreground/10">
            <XCircle className="text-destructive mx-auto mb-2" size={28} />
            <p className="text-2xl font-black text-foreground">{10 - scoreNum}</p>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-card rounded-3xl border-2 border-foreground/10 overflow-hidden">
            {result.details.map((detail, index) => {
              const isCorrect = detail.includes('Correct');
              return (
                <div 
                  key={index}
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {isCorrect ? 'Answered correctly' : detail.split('(')[1]?.replace(')', '') || 'Incorrect'}
                    </p>
                  </div>
